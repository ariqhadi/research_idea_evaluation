...
def plan_generation_method(
    ...
    prompt = (
        "You are an expert researcher in AI and your job is to expand a brief project idea into a full project proposal with detailed experiment plans so that your students can follow the steps and execute the full project. I will provide you with an idea on the topic of: "
        + topic_description
        + ".\n\n"
    )
...
if __name__ == "__main__":
    ...
    with open(PROMPTS_DIR / "experiment_plan_examples_prompting.txt", "r") as f:
        demo_examples = f.read().strip()

    with open(os.path.join(args.idea_dedup_cache_dir, args.cache_name + ".json")) as f:
    ...
    for idea_name in tqdm(idea_names):
        ...
        cache_file = os.path.join(
            cache_dir, "_".join(idea_name.lower().split()) + ".json"
        )
